    ...
    <div className="max-w-3xl mx-auto px-4 py-16">
      ...
      {sent ? (
        <div className="bg-[var(--success-subtle)] border border-[var(--success)]/25 rounded-[var(--r-lg)] p-6 text-center">
          <p className="text-[var(--success)] font-semibold text-lg">Message sent!</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 max-w-lg">
          <div>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="w-full px-4 py-2.5 rounded-lg bg-[var(--surface)] border border-[var(--border)] text-sm text-[var(--foreground)] placeholder:text-[var(--muted)] focus:outline-none focus:border-[var(--accent-border)] focus:ring-1 focus:ring-[var(--accent)] transition-all" placeholder="Your name" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-[var(--foreground)] mb-1.5">Email</label>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full px-4 py-2.5 rounded-lg bg-[var(--surface)] border border-[var(--border)] text-sm text-[var(--foreground)] placeholder:text-[var(--muted)] focus:outline-none focus:border-[var(--accent-border)] focus:ring-1 focus:ring-[var(--accent)] transition-all" placeholder="your@email.com" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-[var(--foreground)] mb-1.5">Message</label>
            <textarea value={message} onChange={(e) => setMessage(e.target.value)} required rows={5} className="w-full px-4 py-2.5 rounded-lg bg-[var(--surface)] border border-[var(--border)] text-sm text-[var(--foreground)] placeholder:text-[var(--muted)] focus:outline-none focus:border-[var(--accent-border)] focus:ring-1 focus:ring-[var(--accent)] transition-all resize-y" placeholder="How can we help?" />
          </div>
          <button type="submit" className="px-6 py-2.5 bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white rounded-lg text-sm font-medium transition-all active:scale-95">
            Send Message
          </button>
        </form>
      )}

      <div className="mt-12 pt-8 border-t border-[var(--border)] space-y-8 text-sm leading-[1.75] text-[var(--muted)]">
        <section>
        ...
        <p className="text-sm text-[var(--muted)]">
          Email: <a href="mailto:saqibbostan83@gmail.com" className="text-[var(--accent)] hover:text-[var(--accent)] transition-colors">saqibbostan83@gmail.com</a>
        </p>
        <p className="text-sm text-[var(--muted)] mt-1">
          Advertising: <a href="mailto:saqibbostan83@gmail.com?subject=Advertising" className="text-[var(--accent)] hover:text-[var(--accent)] transition-colors">saqibbostan83@gmail.com</a>
        </p>
        <p className="text-sm text-[var(--muted)] mt-1">
          Affiliates: <a href="mailto:saqibbostan83@gmail.com?subject=Affiliate" className="text-[var(--accent)] hover:text-[var(--accent)] transition-colors">saqibbostan83@gmail.com</a>
        </p>
      </div>
    </div>
  );
