  ...
  return (
    <article className="max-w-3xl mx-auto px-4 py-12">
      ...
      <div className="text-sm text-[var(--muted)] space-y-5 leading-relaxed">
        <p>Have a password-protected PDF that you need to edit, print, or share? If you know the password, removing it is straightforward. Our <a href="/unlock" className="text-[var(--accent)] underline">free PDF unlock tool</a> lets you <strong>remove password from PDF</strong> instantly — all in your browser, no uploads, no signup.</p>
        ...
        <div className="bg-[var(--premium-subtle)] border border-[var(--premium-border)] dark:border-[var(--premium-border)] rounded-[var(--r-lg)] p-4 text-sm">
          <p className="font-semibold text-[var(--premium)] mb-1">⚠ Important</p>
          <p className="text-[var(--premium)] dark:text-[var(--premium)]">You should only unlock PDFs you own or have explicit permission to access. Unlocking protected files without authorization may violate copyright or privacy laws.</p>
        </div>

        <h2 className="text-xl font-bold text-[var(--foreground)] pt-4">How to Unlock a PDF (3 Steps)</h2>
        <ol className="list-decimal pl-5 space-y-3">
          <li><strong>Go to the unlock tool</strong> — Visit our <a href="/unlock" className="text-[var(--accent)] underline">free PDF unlock page</a>.</li>
        ...
        <p>If you need to go the other direction — adding a password to a PDF — check out our <a href="/protect" className="text-[var(--accent)] underline">password protect tool</a>. You can encrypt any PDF with a strong password in the same client-side, private workflow.</p>
        ...
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[var(--r-lg)] p-6 mt-6">
          ...
          <a href="/unlock" className="inline-block px-5 py-2.5 bg-[var(--accent)] text-white font-medium rounded-[var(--r-lg)] text-sm hover:bg-[var(--accent-hover)] transition">Unlock Your PDF Now →</a>
        </div>
      </div>
    </article>
  );
