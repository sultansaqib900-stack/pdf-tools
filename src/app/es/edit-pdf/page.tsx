      ...
      {!file ? (
        <div onDrop={onDrop} onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)}
          ...
          <label htmlFor="fileInput" className="cursor-pointer flex flex-col items-center gap-3">
            ...
            <span className="text-[var(--accent)] font-medium hover:underline">Haz clic para seleccionar un PDF o arrastra aquí</span>
          </label>
        </div>
      ) : (
        <div className="bg-[var(--surface)] rounded-[var(--r-lg)] border border-[var(--border)] p-4">
          <div className="flex flex-wrap items-center gap-2 mb-4 pb-4 border-b border-[var(--border)]">
            ...
            {selectedId && <button onClick={deleteSelected} className="ml-auto px-3 py-1.5 text-xs bg-[var(--danger)] text-white rounded-lg hover:opacity-90">Eliminar</button>}
          </div>

          <div className="flex items-center gap-2 mb-3 text-sm">
            <span className="text-[var(--muted)]">Página {currentPage + 1} de {numPages}</span>
            <div className="flex gap-1 ml-auto">
              <button onClick={() => setCurrentPage((p) => Math.max(0, p - 1))} disabled={currentPage === 0} className="px-2 py-1 text-xs bg-[var(--background)] border border-[var(--border)] rounded disabled:opacity-40">Anterior</button>
              <button onClick={() => setCurrentPage((p) => Math.min(numPages - 1, p + 1))} disabled={currentPage >= numPages - 1} className="px-2 py-1 text-xs bg-[var(--background)] border border-[var(--border)] rounded disabled:opacity-40">Siguiente</button>
            </div>
          </div>
          ...
          {!isPremium() && (
            <p className="mt-2 text-center text-xs text-[var(--muted)]">
              Usuarios gratuitos limitados a 10MB.<a href="/premium" className="text-[var(--accent)] font-medium hover:underline ml-1">Actualiza para 100MB y sin espera</a>
            </p>
          )}
