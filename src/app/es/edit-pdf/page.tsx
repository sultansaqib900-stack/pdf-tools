...
export default function EsEditPdfPage() {
  ...
  const handleMouseUp = (e: React.MouseEvent) => {
    ...
    if (tool === "text") {
      ...
      newBoxes[currentPage] = [...newBoxes[currentPage], {
        ...
        text: "Escribe aquí", fontSize: currentFontSize, color: currentColor,
      }];
  ...
  const savePdf = useCallback(async () => {
    ...
    try {
      ...
      a.href = url; a.download = "editado.pdf"; a.click();
      URL.revokeObjectURL(url);
      trackExport("editado.pdf", "Edit PDF", pdfBytes.byteLength);
      setSuccess(true);
    } catch {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <SoftwareAppJsonLd name="Editar PDF - Editor PDF Gratuito Online" description="Edita archivos PDF online gratis. Añade texto, formas y dibujos a cualquier PDF." url="https://allaboutpdfediting.xyz/es/edit-pdf" />
      <HowToJsonLd name="Editar PDF" description="Añade texto y formas a documentos PDF" steps={[{name:"Subir PDF",text:"Selecciona un PDF para editar"},{name:"Añadir contenido",text:"Usa la barra de herramientas para añadir cuadros de texto, rectángulos, círculos o líneas"},{name:"Descargar",text:"Guarda tu PDF editado"}]} />
      <BreadcrumbJsonLd items={[{ name: "Inicio", item: "https://allaboutpdfediting.xyz/es" }, { name: "Editar PDF", item: "https://allaboutpdfediting.xyz/es/edit-pdf" }]} />
      <FaqPageJsonLd questions={rc?.faqs} />
      <AiSummaryJsonLd name="Editar PDF" summary="Edita archivos PDF online — añade cuadros de texto, formas y dibujos a cualquier PDF" category="Editor" inputType="PDF" outputType="PDF" processing="lado-del-cliente" price="free" features={["Añadir cuadros de texto","Dibujar formas","Edición multipágina","Procesamiento local","Herramienta gratuita"]} limits="Archivos hasta 10MB" />
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-[var(--foreground)] mb-2">Editar PDF</h1>
        <p className="text-[var(--muted)]">Añade texto, formas y dibujos a cualquier PDF.</p>
      </div>
      <ToolInfo name="Editar PDF" description="Edita PDFs directamente en tu navegador sin subidas. Añade cuadros de texto, rectángulos, círculos y líneas a cualquier página." />
      ...
      {!file ? (
        <div onDrop={onDrop} onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)}
          ...
          <label htmlFor="fileInput" className="cursor-pointer flex flex-col items-center gap-3">
            ...
            <span className="text-indigo-500 font-medium hover:underline">Haz clic para seleccionar un PDF o arrastra aquí</span>
          </label>
        </div>
      ) : (
        <div className="bg-[var(--card)] rounded-xl border border-[var(--card-border)] p-4">
          <div className="flex flex-wrap items-center gap-2 mb-4 pb-4 border-b border-[var(--card-border)]">
            {(["select", "text", "rect", "circle", "line"] as Tool[]).map((t) => (
              <button key={t} onClick={() => { setTool(t); setSelectedId(null); }}
                ...
                {t === "select" ? "↖ Seleccionar" : t === "text" ? "T Texto" : t === "rect" ? "▭ Rect" : t === "circle" ? "○ Circ" : "╱ Línea"}
              </button>
            ))}
            <div className="w-px h-6 bg-[var(--card-border)] mx-1" />
            <input type="color" value={currentColor} onChange={(e) => setCurrentColor(e.target.value)} className="w-8 h-8 rounded cursor-pointer" title="Color de trazo" />
            <input type="color" value={currentFill} onChange={(e) => setCurrentFill(e.target.value)} className="w-8 h-8 rounded cursor-pointer" title="Color de relleno" />
            ...
            {selectedId && <button onClick={deleteSelected} className="ml-auto px-3 py-1.5 text-xs bg-red-500 text-white rounded-lg hover:bg-red-600">Eliminar</button>}
          </div>

          <div className="flex items-center gap-2 mb-3 text-sm">
            <span className="text-[var(--muted)]">Página {currentPage + 1} de {numPages}</span>
            <div className="flex gap-1 ml-auto">
              <button onClick={() => setCurrentPage((p) => Math.max(0, p - 1))} disabled={currentPage === 0} className="px-2 py-1 text-xs bg-[var(--background)] border border-[var(--card-border)] rounded disabled:opacity-40">Anterior</button>
              <button onClick={() => setCurrentPage((p) => Math.min(numPages - 1, p + 1))} disabled={currentPage >= numPages - 1} className="px-2 py-1 text-xs bg-[var(--background)] border border-[var(--card-border)] rounded disabled:opacity-40">Siguiente</button>
            </div>
          </div>
          ...
          <ProgressBar processing={processing} fileSize={file?.size || 0} label="Guardando PDF editado..." />
          ...
          <button onClick={convert} disabled={processing || showTimer || saving}
            ...
            {processing ? "Guardando..." : "Descargar PDF Editado"}
          </button>

          {!isPremium() && (
            <p className="mt-2 text-center text-xs text-[var(--muted)]">
              Usuarios gratuitos limitados a 10MB.<a href="/premium" className="text-indigo-500 font-medium hover:underline ml-1">Actualiza para 100MB y sin espera</a>
            </p>
          )}

          {error && <ErrorBanner message={error} onRetry={runConvert} onDismiss={() => setError(null)} />}
          <SuccessAnimation show={success} message="¡PDF guardado!" />
        </div>
      )}

      <div className="max-w-3xl mx-auto mt-12 pt-8 border-t border-[var(--card-border)]">
        <h2 className="text-xl font-bold text-[var(--foreground)] mb-3">Acerca de Editar PDF</h2>
        <div className="text-sm text-[var(--muted)] space-y-3 leading-relaxed">
          <p>Edita archivos PDF online con nuestro editor PDF gratuito. Añade cuadros de texto, rectángulos, círculos y líneas a cualquier página. Perfecto para rellenar formularios, añadir notas, marcar documentos o crear diagramas — todo en tu navegador sin subidas al servidor.</p>
          <p>Tu PDF original se mantiene privado. Toda la edición ocurre del lado del cliente usando pdf-lib. Simplemente sube, edita y descarga tu PDF modificado.</p>
        </div>
      </div>
