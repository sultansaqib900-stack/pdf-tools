      ...
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[var(--r-lg)] p-6 space-y-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <button key={m.value} onClick={() => setMode(m.value)} className={`p-4 rounded-[var(--r-lg)] border text-center transition ${mode === m.value ? "border-[var(--accent-border)] bg-[var(--accent-subtle)] bg-[var(--accent-subtle)] ring-1 ring-[var(--accent)]" : "border-[var(--border)] hover:border-[var(--accent-border)]"}`}>
        ...
        <button onClick={transform} disabled={!file || processing} className="w-full py-3 bg-[var(--premium)] text-white font-bold rounded-[var(--r-lg)] hover:opacity-90 disabled:opacity-40 transition">
          {processing ? "Transforming..." : "Transform PDF"}
        </button>
      </div>

      {success && <div className="mt-4 p-4 bg-[var(--success-subtle)] border border-[var(--success)]/25 rounded-[var(--r-lg)] text-center text-sm text-[var(--success)]">✅ Transformed PDF downloading!</div>}
